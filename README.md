...
stagehand uses playwright which launches a headless browser. this freaks out if you attempt to run as root. because of this, there's a service in `docker-compose.yml` called `article-worker` (I know... bad name lol). `article-works` spins up a docker contains that hosts the worker for stagehand to make this all work. 
